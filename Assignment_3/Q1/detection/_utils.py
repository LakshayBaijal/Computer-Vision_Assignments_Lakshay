...
@torch.jit._script_if_tracing
def encode_boxes_with_angle(reference_boxes: Tensor, proposals: Tensor, weights: Tensor) -> Tensor:
    """
    Encode a set of proposals with respect to some reference boxes (includes angle).
    
    Args:
        reference_boxes (Tensor): reference boxes with angle (N, 5) - [x1, y1, x2, y2, theta]
        proposals (Tensor): boxes to be encoded (N, 5) - [x1, y1, x2, y2, theta]
        weights (Tensor[5]): the weights for ``(x, y, w, h, angle)``
    """
    wx = weights[0]
    wy = weights[1]
    ww = weights[2]
    ...
    proposals_x2 = proposals[:, 2].unsqueeze(1)
    proposals_y2 = proposals[:, 3].unsqueeze(1)
    proposals_angle = proposals[:, 4].unsqueeze(1)

    reference_boxes_x1 = reference_boxes[:, 0].unsqueeze(1)
    reference_boxes_y1 = reference_boxes[:, 1].unsqueeze(1)
    reference_boxes_x2 = reference_boxes[:, 2].unsqueeze(1)
    reference_boxes_y2 = reference_boxes[:, 3].unsqueeze(1)
    reference_boxes_angle = reference_boxes[:, 4].unsqueeze(1)
    ...
    # Angle encoding with normalization to [-180, 180]
    angle_diff = reference_boxes_angle - proposals_angle
    angle_diff = torch.where(angle_diff > 180, angle_diff - 360, angle_diff)
    angle_diff = torch.where(angle_diff < -180, angle_diff + 360, angle_diff)
    targets_angle = wangle * angle_diff / 180.0

    targets = torch.cat((targets_dx, targets_dy, targets_dw, targets_dh, targets_angle), dim=1)
    return targets
...
class BoxCoderWithAngle:
    """
    Box coder for oriented bounding boxes (with angle).
    Encodes/decodes 5D boxes: [x1, y1, x2, y2, theta]
    """

    def __init__(
        self, weights: Tuple[float, float, float, float, float], bbox_xform_clip: float = math.log(1000.0 / 16)
    ) -> None:
        """
        Args:
            weights (5-element tuple): (wx, wy, ww, wh, wangle)
            bbox_xform_clip (float)
        """
        self.weights = weights
        self.bbox_xform_clip = bbox_xform_clip
    ...
    def encode_single(self, reference_boxes: Tensor, proposals: Tensor) -> Tensor:
        """
        Encode 5D boxes (with angle).

        Args:
            reference_boxes (Tensor): reference boxes [N, 5]
            proposals (Tensor): boxes to be encoded [N, 5]
        """
        dtype = reference_boxes.dtype
        device = reference_boxes.device
        weights = torch.as_tensor(self.weights, dtype=dtype, device=device)
        targets = encode_boxes_with_angle(reference_boxes, proposals, weights)
        return targets
    ...
    def decode_single(self, rel_codes: Tensor, boxes: Tensor) -> Tensor:
        """
        From a set of original boxes and encoded relative box offsets,
        get the decoded 5D boxes (with angle).

        Args:
            rel_codes (Tensor): encoded boxes
            boxes (Tensor): reference boxes [N, 5].
        """
        ...
        wx, wy, ww, wh, wangle = self.weights
        dx = rel_codes[:, 0::5] / wx
        dy = rel_codes[:, 1::5] / wy
        dw = rel_codes[:, 2::5] / ww
        dh = rel_codes[:, 3::5] / wh
        dangle = rel_codes[:, 4::5] / wangle
        ...
        # Decode angle
        pred_angle = dangle * 180.0 + angles[:, None]
        # Normalize angle to [0, 360)
        pred_angle = torch.fmod(pred_angle, 360.0)
        pred_angle = torch.where(pred_angle < 0, pred_angle + 360, pred_angle)
        
        pred_boxes = torch.stack((pred_boxes1, pred_boxes2, pred_boxes3, pred_boxes4, pred_angle), dim=2).flatten(1)
        return pred_boxes
