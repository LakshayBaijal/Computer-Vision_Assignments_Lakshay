...
class FastRCNNPredictorWithAngle(nn.Module):
    """
    Classification + bounding box regression layers for Fast R-CNN with angle prediction.
    ...
    def __init__(self, in_channels, num_classes):
        super().__init__()
        self.cls_score = nn.Linear(in_channels, num_classes)
        self.bbox_pred = nn.Linear(in_channels, num_classes * 5)  # 4 coords + 1 angle
