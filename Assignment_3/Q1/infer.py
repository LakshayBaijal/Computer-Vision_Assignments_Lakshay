...
def get_rotated_iou(det, gt):
    """
    Compute IoU for oriented bounding boxes using cv2.
    det: [x1, y1, x2, y2, theta] - detection box
    gt: [x1, y1, x2, y2, theta] - ground truth box
    theta is in degrees
    """
    det_x1, det_y1, det_x2, det_y2, det_theta = det
    gt_x1, gt_y1, gt_x2, gt_y2, gt_theta = gt
    
    # Convert to center, width, height, angle format for cv2.boxPoints
    det_w = det_x2 - det_x1
    det_h = det_y2 - det_y1
    ...
    gt_w = gt_x2 - gt_x1
    gt_h = gt_y2 - gt_y1
    gt_cx = gt_x1 + gt_w / 2.0
    gt_cy = gt_y1 + gt_h / 2.0
    
    # Get corner points for both boxes
    det_rect = ((det_cx, det_cy), (det_w, det_h), det_theta)
    gt_rect = ((gt_cx, gt_cy), (gt_w, gt_h), gt_theta)
    
    det_points = cv2.boxPoints(det_rect)
    gt_points = cv2.boxPoints(gt_rect)
    
    # Use shapely for polygon intersection if available, otherwise approximate
    try:
        from shapely.geometry import Polygon
        det_poly = Polygon(det_points)
        gt_poly = Polygon(gt_points)
        
        intersection_area = det_poly.intersection(gt_poly).area
        union_area = det_poly.union(gt_poly).area
...
def compute_map(det_boxes, gt_boxes, iou_threshold=0.5, method='area', use_angle=False):
    # det_boxes = [
    #   {
    ...
    for idx, label in enumerate(gt_labels):
        ...
        for det_idx, (im_idx, det_pred) in enumerate(cls_dets):
            ...
            iou_func = get_rotated_iou if use_angle else get_iou
            for gt_box_idx, gt_box in enumerate(im_gts):
                if use_angle:
                    gt_box_iou = iou_func(det_pred[:-1], gt_box)
                else:
                    gt_box_iou = iou_func(det_pred[:-1], gt_box)
...
def load_model_and_dataset(args):
    ...
    checkpoint_path = args.checkpoint_path if args.checkpoint_path else os.path.join(
        train_config['task_name'], 'tv_frcnn_r50fpn_' + train_config['ckpt_name']
    )
    faster_rcnn_model.load_state_dict(torch.load(checkpoint_path, map_location=device))

    return faster_rcnn_model, st, test_dataset, use_angle
...
def infer(args):
    ...
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    faster_rcnn_model, voc, test_dataset, _ = load_model_and_dataset(args)
    ...
    for sample_count in tqdm(range(10)):
        ...
        for idx, box in enumerate(target['bboxes']):
            ...
            if len(box_data) == 5:
                x1, y1, x2, y2, theta = box_data
                w = x2 - x1
                h = y2 - y1
                cx = x1 + w / 2.0
                cy = y1 + h / 2.0
                rect = ((cx, cy), (w, h), float(theta))
                points = cv2.boxPoints(rect)
                points = points.astype(np.int32)
                cv2.polylines(gt_im, [points], True, [0, 255, 0], 2)
                cv2.polylines(gt_im_copy, [points], True, [0, 255, 0], 2)
                x1, y1 = int(x1), int(y1)
            else:
        ...
        for idx, box in enumerate(boxes):
            box_data = box.detach().cpu().numpy()
            if len(box_data) == 5:  # OBB format [x1, y1, x2, y2, theta]
                x1, y1, x2, y2, theta = box_data
                # Draw rotated box
                w = x2 - x1
                h = y2 - y1
                ...
                cv2.polylines(im_copy, [points], True, [0, 0, 255], 2)
            else:  # Regular box format [x1, y1, x2, y2]
                x1, y1, x2, y2 = box_data
                x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
            ...
            text_w, text_h = text_size
            if len(box_data) == 5:
                x1_text, y1_text = int(box_data[0]), int(box_data[1])
            else:
                x1_text, y1_text = int(box_data[0]), int(box_data[1])
            cv2.rectangle(im_copy, (x1_text, y1_text), (x1_text + 10 + text_w, y1_text + 10 + text_h), [255, 255, 255], -1)
            cv2.putText(im, text=text,
                        org=(x1_text + 5, y1_text + 15),
                        thickness=1,
                        fontScale=1,
                        color=[0, 0, 0],
                        fontFace=cv2.FONT_HERSHEY_PLAIN)
...
def evaluate_map(args):
    faster_rcnn_model, voc, test_dataset, use_angle = load_model_and_dataset(args)
    gts = []
    preds = []
    for im, target, fname in tqdm(test_dataset):
        ...
        for idx, box in enumerate(boxes):
            box_vals = box.detach().cpu().numpy()
            label = labels[idx].detach().cpu().item()
            score = scores[idx].detach().cpu().item()
            label_name = voc.idx2label[label]
            if len(box_vals) == 5:
                x1, y1, x2, y2, theta = box_vals
    ...
    mean_ap, all_aps = compute_map(preds, gts, method='interp', use_angle=use_angle)
    print('Class Wise Average Precisions')
    for idx in range(len(voc.idx2label)):
        label_name = voc.idx2label[idx]
        ap_val = all_aps.get(label_name, float('nan'))
        print('AP for class {} = {:.4f}'.format(label_name, ap_val))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', dest='checkpoint_path',
                        default=None, type=str, help='Path to checkpoint')
